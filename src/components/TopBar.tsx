...
export interface TopBarProps {
  logo: ReactNode
  title?: ReactNode
  /** LNB(NavRail)를 접고 펼치는 토글 버튼. onToggleCollapsed를 생략하면 버튼 자체를 그리지 않는다
   * (LNB 없이 탑 영역만 쓰는 화면용). */
  collapsed?: boolean
 ...
 */
export function TopBar({ logo, title, collapsed = false, onToggleCollapsed, right, railWidth = 64 }: TopBarProps) {
  return (
    <header className={clsx('flex h-12 shrink-0 items-center gap-2 bg-[var(--ds-surface-sunken)] pr-3', !onToggleCollapsed && 'pl-3')}>
      {onToggleCollapsed && (
      ...
      {title && <span className="text-sm font-bold tracking-tight text-[var(--ds-text)]">{title}</span>}
